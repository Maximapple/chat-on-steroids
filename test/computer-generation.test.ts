...
describe.each(['stdio', 'addon'] as const)('Desktop reply provenance (%s)', (transport) => {
  ...
  it('retains completed input evidence if the subsequent screenshot fails', async () => {
    // A window-bound frame, because physical text input is not allowed to aim itself: the fence
    // wants an exact target, and a frame taken of window 77 is one. Taken before the failure is
    // armed, so the mock still belongs to the capture this test is about.
    const framed = await computer.getWindowState({ window: 77 });
    vi.spyOn(fs, 'stat').mockRejectedValueOnce(new Error('fixture image unavailable'));
    await expect(computer.actAndCapture([{ type: 'type', text: 'already sent' }], { frameId: framed.screenshot!.frameId, capture: { window: 77 } }))
