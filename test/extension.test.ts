...
describe('worker settings authority', () => {
  ...
  it('forwards the source give-up, so a handoff ChatGPT never took can be ended', async () => {
    ...
    const worker = loadWorker({ local: new FakeStorageArea(paired), session: new FakeStorageArea(), fetch,
      // 2.0.7 reads Chrome's own tab state before accepting a /compact route, so this harness has
      // to answer with the tab the message claims to come from rather than refusing the lookup.
      tabsGet: async (id: number) => ({ id, url: `https://chatgpt.com/c/${CHAT}` }) });
    await worker.registerTab(44);
    await worker.send({ type: 'bind', conversationId: CHAT }, 44);
    const token = '0123456789abcdef0123456789abcdef';
  ...
  it('forwards every compaction checkpoint the content script can send', async () => {
    ...
    // A third shape arrived with 2.0.7: a count that qualifies another checkpoint rather than
    // being one. It is declared as a map from the field to the checkpoint it describes, so read
    // its keys — leaving it out here would put this guard's own blind spot back.
    const listedKeys = (name: string): string[] => {
      const block = new RegExp(`const ${name} = \{([^}]*)\}`).exec(worker);
      expect(block, `${name} is not a literal map any more`).not.toBeNull();
      return [...block![1]!.matchAll(/(\w+)\s*:/g)].map((match) => match[1]!);
    };
    const forwarded = new Set([
      ...listed('COMPACT_CHECKPOINT_FLAGS'),
      ...listed('COMPACT_CHECKPOINT_TEXT'),
      ...listedKeys('COMPACT_CHECKPOINT_COUNTS')
    ]);
    ...
    // From 2.1.0 the handler also carries the command and client of a destination checkpoint,
    // by the checkpoint's own shape rather than through the three lists.
    ...
    expect(
      ...
      `content.js sends ${unroutable.join(', ')} on /compact, and background.js forwards none of it — ` +
        'add each to COMPACT_CHECKPOINT_FLAGS, COMPACT_CHECKPOINT_TEXT or COMPACT_CHECKPOINT_COUNTS'
    ).toEqual([]);
