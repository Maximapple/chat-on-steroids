...
describe('delivering a bootstrap', () => {
  ...
  it('stops opening fresh chats for a brief ChatGPT keeps refusing, and says so', async () => {
    ...
    setStuckNotifier((title, body) => { told.push({ title, body }); });
    try {
      await pair();
      ...
      for (let attempt = 1; attempt <= 4; attempt++) {
        ...
        if (attempt > 3) {
          // Nothing further is opened, and the person hears why rather than finding a dead session.
      ...
      expect(told).toHaveLength(1);
      expect(told[0]!.body).toMatch(/refused the handoff brief in 4 fresh chats/i);
      expect(told[0]!.body).toMatch(/nothing was sent twice/i);
    } finally {
      setStuckNotifier(null);
    }
  });
...
describe('unattributed activity recovery', () => {
  ...
  it('rescues a compaction restored from the previous run, which no page will report', async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(PRO_SILENCE_MS + 15_000);
  ...
  it.each([1, 2, 3])('wakes the extension at the exact deadline for %i suspects without a status poll', async count => {
    ...
    try {
      ...
      socket?.terminate();      vi.useRealTimers();
    }
  });
  ...
  it('reports a chat whose last turn failed and was followed by nothing', async () => {
    ...
    try {
      ...
      //
      // Swept the way the app sweeps: `STALE_SWARM_SWEEP_MS` runs this pass every thirty
      // seconds regardless of any grant's own deadline, and the retire path — the one that
      // reaches the desktop — lives only in that pass. A fixture that drives the silence
      // timer alone measures a watchdog the app does not run by itself.
      for (let round = 0; round < 6; round++) {
        await vi.advanceTimersByTimeAsync(PRO_SILENCE_MS + 15_000);
        await sweepStaleSwarm(Date.now());
        const repair = await maintenance();
  ...
  it('restores browser recovery to a chat whose continuation predates this run', async () => {
    ...
    try {
      ...
      await sweepStaleSwarm(Date.now());
      // Either route is a pass, and the comment above has said so since this test was written:
      // what must not happen is the chat going unrecovered for the ticket's whole six-hour life.
      // A restored ticket is watched again rather than skipped by the floor, so the compaction
      // pickup used to get there first; after upstream's 2026-09-17 recovery rework the
      // handed-back candidate is considered on the same pass and wins by ordering. Pinning one
      // of the two would be pinning that ordering, which is not what this is about.
      const recovery = await maintenance();
      expect(recovery).toMatchObject({ conversationId: OTHER });
      expect(['compaction', 'silence']).toContain(recovery!.reason);
  ...
  it('leaves a chat alone while one of its tool calls is still running', async () => {
    ...
    try {
      ...
      release?.();
      await inFlight;
      await vi.advanceTimersByTimeAsync(PRO_SILENCE_MS + 3 * 60_000);
      await sweepStaleSwarm(Date.now());
