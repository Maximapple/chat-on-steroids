...
describe('surface boundaries', () => {
   ...
   */
  it('offers giving a page back alongside the actions that drive it', async () => {
    everything();
    const tools = toolList(await desktop('tools/list'));
    // 2.1.13 split the single `browser` tool with an action list into one tool per job. The
    // concern is unchanged and still belongs somewhere: taking a tab must be releasable from
    // the schema, not only from the extension popup.
    const tabs = tools.find((tool) => tool.name === 'browser_tabs');
    expect(tabs, 'the browser_tabs tool').toBeDefined();
    const schema = JSON.stringify(tabs);
    for (const action of ['list', 'attach', 'release', 'close']) {
      expect(schema, action).toContain(`"${action}"`);
    }
    for (const tool of ['browser_snapshot', 'browser_navigate', 'browser_action']) {
      expect(toolNames(await desktop('tools/list')), tool).toContain(tool);
    }
  });
  ...
  it('remembers a capability switched on after the endpoint started', async () => {
    ...
    expect(toolNames(await desktop('tools/list'))).toContain('browser_tabs');
    expect(toolNames(await desktop('tools/list'))).toContain('browser_tabs');
  });
