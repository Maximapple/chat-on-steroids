...
describe('a worker that is sleeping', () => {
  ...
  it('discards a restored history that claims a conversation another owner already holds', async () => {
    ...
    // as an empty swarm rather than handing back a half-owned run. 2.1.0 answers that with an
    // empty state instead of an error — the fact asserted here is the same one.
    expect(swarmStateForCaller(primeB).agents).toEqual([]);
