 ...
 * a classic script, where an import statement is a parse error. Strip the statement and supply
 * the binding through the context instead — the same rewrite test/extension.test.ts performs,
 * and for the same reason: the import is static so that a broken browser_* message fails in a
 * real browser rather than passing here. Fail loudly if it stops looking the way this expects,
 * rather than quietly evaluating a worker with a missing driver.
 */
...
// Stubbed: these tests exercise outbox maintenance, never a debugger session. See verify:browser.
const browserDriverModule = {
  installBrowserDriverLifecycle() {},
  sweepStaleDrivenGroups: async () => {}
};
...
async function worker(inputs: Array<{ id: string; conversationId: string | null; directTurn?: { id: string; startedAt: number }; supersededConversationId?: string }>, modelCatalogRequest?: { nonce: string; expiresAt: number }, priorLocal: Record<string, unknown> = {}) {
  ...
  const context = vm.createContext({
    ...
    fetch, URL, URLSearchParams, AbortController, setTimeout, clearTimeout, TextEncoder, console, browserDriverModule
  });
