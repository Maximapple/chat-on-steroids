...
async function harness(
  ...
  window.chrome = {
    ...
    storage: {
      // No production reader depends on a seeded value; a test that needs one overrides
      // `local.get` from its own `before()` hook.
      local: {
        async get() {
          return {};
        }
...
describe('the Compact & resume control', () => {
  ...
  it('interrupts the live turn, waits for local tools, then prompts this same chat', async () => {
    ...
    expect(submitted).toContain('the brief itself');    expect(sends()).toBe(1);
...
describe('the field above the composer', () => {
  ...
  /**
   * The caption describes another process's memory, so it is only as good as the last answer.
   *
   * Restarting the app under a live turn left this panel reading a pending-tool count for over
   * ten minutes, across a turn that had since completed: the calls died with the old process and
   * nothing expired the number, so the page kept asserting a fact about a process that no longer
   * existed. An app restart is an ordinary event — the updater performs one.
   *
   * 2.0.7 settled this the strict way rather than on a trust clock: a pull that does not answer
   * clears the projection outright. A brief blip therefore drops the caption and the next pull
   * brings it back, which is the trade taken deliberately — saying nothing is honest, and saying
   * something stale about another process is not.
   */
  it('stops asserting a pending-tool count once the app has stopped confirming it', async () => {
    live = await harness('https://chatgpt.com/c/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee', {
      // `since` is on the script's own clock, which this harness fakes and only advance() moves;
      // zero is simply "long enough ago" once the wait threshold below has been stepped past.
      activity: () => ({
        ok: true,
        data: { entries: [], stream: [], nextSince: 0, progress: { tools: { count: 2, since: 0 } } }
      })
    });
    await live.hook.pullActivity();
    await settle();
    ...
    live.hook.injectStage();
    expect(live.document.querySelector('.clf-stage')?.textContent).toContain('Waiting for 2 local tools to finish');

    // The app goes away. The very next unanswered pull retires the claim rather than leaving it
    // standing for the rest of the turn.
...
describe('the fresh chat the app opened', () => {
  ...
  it('redeems the one command its URL names and crosses the durable destination fence', async () => {
    ...
    expect(submitted).toContain('Handoff: h-1');    expect(live.sent.some((message) => message.type === 'compact' && message.destinationAttempt === true)).toBe(true);
  ...
  it('acquires its id when a Project route names the fresh chat', async () => {
    ...
    expect(submitted).toContain('Handoff: h-project');    expect(live.sent.some((message) => message.type === 'compact' && message.destinationAttempt === true)).toBe(true);
...
(You are a worker agent in a Chat On Steroids multi-agent run.)`,
    ...
    expect(live.submitted.join('\n')).toContain(task);
    expect(submitted).toContain(task);    expect(live.sent.filter((message) => message.type === 'ack')).toEqual([
      {
        type: 'ack',
        id: 'cmd-10',
        status: 'sent',
...
describe('issues #34/#35 hidden worker and proactive wait status', () => {
  ...
  it('renders authoritative wait state only after a quiet threshold and never over live prose', async () => {
    live = await harness();
    // 2.0.7 moved the reasoning into the app: the page is handed a finished projection of what
    // the turn is waiting on and only formats it. The properties asserted here are unchanged —
    // a quiet threshold before any caption, the exact worker wording, and never inventing a
    // remote dependency from generation alone — but they are now driven through that shape.
    const NOW = 1_000_000;
    const view = (over: Record<string, unknown>) =>
      live!.hook.stageView({ job: null, goal: null, changedAt: NOW - 5000, generating: false, now: NOW, ...over });

    const workers = { finished: 1, active: 1, failed: 1, names: ['macOS validation'] };

    // Live prose wins: a turn that changed a moment ago gets no caption at all.
    expect(view({ progress: { workers }, changedAt: NOW - 1000 })).toBeNull();

    expect(view({ progress: { workers } })).toMatchObject({
      stage: 'Worker still running: macOS validation',
      detail: '1 finished · 1 running · 1 failed'
    });

    // Generating with nothing else known: say that and nothing more.
    expect(view({ progress: null, generating: true })).toMatchObject({
      stage: 'Still waiting for the current operation to complete'
    });

    expect(view({ progress: { tools: { count: 2, since: NOW - 5000 } }, generating: true })).toMatchObject({
      stage: 'Waiting for 2 local tools to finish'
    });

    // Returned async exec sessions keep their own wording; they are not in-flight tool calls.
    expect(view({ progress: null, backgroundExec: { running: 0, exitedUnread: 3 }, generating: true })).toMatchObject({
  ...
  /**
   * Removed with the 2.1.13 merge, deliberately rather than repaired.
   *
   * It fed the renderer an `assistant_message` with no turnId and expected it on screen.
   * 2.1.13 refuses exactly that: an event the feed cannot name belongs to no turn group,
   * because placing it by position is what made reloads swirl — "the cost of refusing is a
   * row rendered natively; the cost of guessing is a wrong transcript". The hidden-worker
   * stream this covered now renders through ChatGPT's own markup instead.
   */
});
