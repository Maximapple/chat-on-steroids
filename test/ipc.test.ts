...
describe('session IPC contracts', () => {
  ...
  it('gives up an open continuation when the row that owned it is deleted', async () => {
    ...
    // age-pruning — it outlives the app. pendingContinuations() keeps naming its
    ...
    const { openContinuationNow, pendingContinuations, continuationForSession, resetContinuationsForTests } =
    ...
    expect(pendingContinuations().map((entry) => entry.from)).toContain(conversationId);
    ...
    expect(pendingContinuations().map((entry) => entry.from)).not.toContain(conversationId);
    resetContinuationsForTests();
  });
