...
var CLF_DOM = (() => {
    ...
    return safe(() => {
      ...
      if (role === 'user') {
        ...
        const parts = [...node.querySelectorAll('.whitespace-pre-wrap')]
          ...
          .filter(part => !part.hasAttribute?.('data-clf-user-text'))          .map((part) => text(part))
          .filter(Boolean);
  ...
  function captureComposerDraft(value, stillCurrent = () => true, { followRemount = false } = {}) {
    ...
    const readopt = () => {
      ...
      if (!current || current === box || !current.isConnected || compact(current.textContent) !== insertedText) return;
    ...
    const same = () => {
      ...
      return !touched && stillCurrent() && composer() === box && box?.isConnected && compact(box.textContent) === insertedText;
