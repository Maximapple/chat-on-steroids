...
(() => {
  ...
  function finishGeneration(ended, result, publishFinal = true) {
    ...
    if (endedTurnId) {
      ...
      fiberSettled = {
        ...
        // From 2.1.0: the settled thinking failure carries its own reason and the moment it
        // ended, which `isWork` below reads to stop counting rows from before that point.
  ...
  function observe() {
    ...
    if (generating && turn) {
      ...
      if (!thinkingFailure && !stallReported && Date.now() - lastChangeAt > STALL_MS) {
        stallReported = true;
        let text = 'No visible progress for ten minutes. The app could not confirm that this turn finished.';
  ...
  function stageView(input) {
    ...
    if (Number.isFinite(unread) && unread > 0)
      return frame(unread === 1 ? '1 completed background result waiting' : `${unread} completed background results waiting`,
        'Read the final output with write_stdin before starting more background work');
    const running = Number(exec && exec.running);
    if (Number.isFinite(running) && running > 0)
      return frame(running === 1 ? 'Background process still running' : `${running} background processes still running`,
        'The process is still active even though its tool call already returned');
