...
async function loadOnce() {
  ...
  const live = await webext.storage.session.get([    'settled',
    'journal',
    'tabConversations',
    'tabDocuments',
    'tabEpochs',
...
const HANDLERS = {
  ...
  async compact(message, _sender, source) {
    ...
    //
    // From 2.1.0, with the measurement above kept intact: loading *this* leased document is
    // still normal and still earns the permit, but a tab that is on its way to some other
    // address is not the replacement chat this checkpoint claims to come from.
    if (named
    ...
    const result = await call('/compact', {
      ...
      body: JSON.stringify({
        ...
        //
        // From 2.1.0: a destination checkpoint also carries the command and client it belongs
        // to, so the app can tell which attempt is speaking.
  ...
  async focus_tab(message, _sender, source) {
    ...
    try {
      // Select the tab, and stop there. This used to raise its window too, which meant Goal
      // could pull the whole browser in front of whatever the person was doing. 2.0.7 dropped
      // that on purpose: the tab is the thing Goal owns, and the foreground is the user's.
      await webext.tabs.update(source.tab, { active: true });
    } catch {
      ...
      if (!targetUrl) {
        try {
          ...
          targetUrl = typeof tab?.pendingUrl === 'string' && tab.pendingUrl ? tab.pendingUrl :
            typeof tab?.url === 'string' ? tab.url : '';        } catch {
          targetUrl = '';
        }
      }
  ...
  const work = (async () => {
    ...
    for (const entry of [...deferredRevivals]) {
      ...
      try {
        ...
        const created = await webext.tabs.create({ url, active: false });
