...
function nextWriteLazy(snapshot: () => unknown): PendingWrite {
  return { generation: nextGeneration++, value: undefined, snapshot };
}
...
async function flushOne(name: string, slot: PendingWrite): Promise<void> {
  ...
  try {
    ...
    // Materialized once above, right before it is actually needed - not at every call that
