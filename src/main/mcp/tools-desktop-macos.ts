    ...
    reg.register(
      ...
      toolDeclaration('computer', () => ({
        ...
        inputSchema: z
          ...
          .superRefine((input, ctx) => {
            // One UI-changing decision per call. A batch that clicks, then types, then clicks
            // again cannot be read back: the picture returned at the end shows the last state,
            // and nothing says which step produced it. focus/move/wait/clipboard setup may
            // accompany the decision, because none of them is one.
      ...
      async ({ actions, frameId, verify, captureAfter, captureWindow, captureFull, captureMaxWidth, captureCrop }) =>
        guard('computer', async () => {
          ...
          // A mutating call returns a fresh picture by default. Upstream makes the result capture
          // opt-in; this fork's instructions promise the opposite ("Mutating computer calls return
          // a fresh result screenshot by default"), and a model that acts and then cannot see what
          // it did is the failure that promise exists to prevent. `captureAfter: false` still
          // declines it, so the caller keeps the way out.
          ...
          const result = await withBrowserInputHint(parsed, undefined, () => actAndCapture(parsed, {
            frameId,
            verify: parsedVerify,
            capture:
              wantsCapture
                ? {
                    ...
                    full: captureFull,
                    // An automatic capture nobody asked for gets a bounded width, so the picture
                    // a mutating call returns by default cannot dominate the response.
          ...
          const pointer = cursor
            ? cursor.image
              ...
              : cursor.frameId === null
                ...
                // Two different facts, and a model acting on coordinates needs them apart: there is
                // no frame to be in, versus there is one and the pointer is outside it. Collapsed
                // into one sentence, the second reads as the first and invites a coordinate that
                // cannot land.
