...
const DESKTOP: SurfaceDefinition = {
  ...
  cardSummary:
    'Apps, background window screenshots, mouse/keyboard control, the clipboard, and driving web pages in Chrome. Optional — connect it only if you want desktop or browser automation.',
  required: false,
  tools: [...WINDOWS_COMPUTER_METHODS, 'read_clipboard', 'write_clipboard', 'observe', 'computer', 'exec', 'browser']};
...
export function desktopToolNames(caps: Capabilities, platform: NodeJS.Platform = process.platform): string[] {
  // `browser` gates on control exactly as its registration does. It was added to this surface
  // and never added to the name list, so the app undercounted its own tools — "9 total" where
  // the server serves ten — and a display disagreed with a server about what exists.
  const browser = caps.control ? ['browser'] : [];
  if (platform !== 'win32') return [...(caps.screen ? ['observe'] : []), ...(caps.control || caps.clipboardRead || caps.clipboardWrite ? ['computer'] : []), ...browser];
  return [
