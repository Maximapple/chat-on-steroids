...
function desktopInstructions(ctx: ToolContext, platform: NodeJS.Platform): string {
  ...
  const lines = [
    ...
    'Prefer focus(window) over command+tab/alt+tab when the destination window id is known. Never use a fixed sleep as proof that an app switch finished.',    // Waiting was the single most repeated desktop pattern in the recorded sessions: a batch of
