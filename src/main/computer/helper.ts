...
public static class Clf {
  ...
  // The cursor is not composited by any capture API here, so PaintCursor draws it. These came
  // back after a merge dropped them while their only caller survived: the C# then failed to
  // compile at all, which is a dead helper rather than a picture without a pointer.
  ...
  public static void Drag(int[] xs, int[] ys, string button, int durationMs) {
    ...
    uint down, up, data;
    // The data word says which side button: without it back and forward both post as
    // button 0, and a drag with either one drags nothing.
    ButtonFlags(button, out down, out up, out data);
    Move(xs[0], ys[0]);
    try {
      Send(new INPUT[] { Mouse(down, 0, 0, data) });
      // The press has to settle before the pointer leaves, or Explorer's shell drag never starts
      // and the helper reports a drag that moved nothing. QA met exactly that on both platforms.
      ...
      for (int step = 1; step <= steps; step++) {
        ...
        if (remaining > 0) System.Threading.Thread.Sleep(Math.Min(DragStepMs, remaining));
      }
    } finally {
      // And a moment at the destination before the drop, for the same reason: the target has to
      // see the pointer arrive before the button comes up, or the drop lands on nothing.
