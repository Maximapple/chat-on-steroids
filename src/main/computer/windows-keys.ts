      ...
      if (!Named.TryGetValue(name, out key)) {
        if (name.Length >= 2 && name.Length <= 3 && (name[0] == 'F' || name[0] == 'f') && int.TryParse(name.Substring(1), out number) && number >= 1 && number <= 24) key = 0x6F + number;
          ...
          if (!Symbols.TryGetValue(name, out symbol)) {
            if (name.Length != 1 || char.IsControl(name[0]) || char.IsSurrogate(name[0])) throw new ArgumentException("BAD_KEY: unsupported key " + name + ". Accepted: one printable character, F1-F24, ENTER/TAB/ESC/SPACE/BACKSPACE/DELETE, HOME/END/PAGEUP/PAGEDOWN, UP/DOWN/LEFT/RIGHT and their ARROW* names, and the modifiers CTRL/CONTROL, ALT/OPTION, SHIFT and WIN/SUPER/CMD/META.");
            symbol = name[0];
          }
