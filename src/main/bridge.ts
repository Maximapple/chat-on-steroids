...
import {
  ...
  recordNote,
  recordRequestEvidence,  recordProgress,
...
async function handle(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  if (route === '/compact' && req.method === 'POST') {
    ...
    if (body['sourceLost'] === true) {
      ...
      if (!aborted) {
        // Upstream ends the transaction only from the two pre-Send states. A dispatch that
        // was armed and then proved unaccepted - none of send()'s five acceptance signals
        // inside its window, on a chat this flow had already stopped and settled - falls
        // through to here, and refusing it leaves the ticket armed until the six-hour TTL
        // with the chat kept out of browser recovery the whole time. Nothing is re-offered
        // either way; the prompt may still be with ChatGPT and the arming fence exists
        // precisely not to gamble on that. Only the transaction ends.
...
/**
 * How many silence reloads one wedged episode may spend in total, whatever the page calls the
 * trouble each time. Higher than the per-failure budget on purpose: genuinely varied trouble is
 * worth more attempts than identical trouble, and this only stops the unbounded case.
 */
const SILENCE_EPISODE_CEILING = 5;
const silenceProvenUseless = new Map<string, { turnKey: string; error: string; attempts: number; episodeAttempts: number; told: boolean }>();
...
function queueBrowserRecovery(
  ...
  if (reason === 'assistant-error') {
    const spent = turnRepairSpent.get(conversationId);
    // 2.1.13's identity, kept: the budget is charged to the question that earned the reload
    // rather than to the turn it happened to run in. A wedged chat mints a new turn id for a
    // generation that never ended, and a turn-keyed budget hands each of those a fresh reload —
    // twenty-four rounds produced ten of them here. The user message does not move.
    if (!assistantSource) return declineRecovery(conversationId, reason, 'no answer this reload could belong to');
    if (spent?.sessionId === sessionId && assistantSource.key === spent.turnKey) {
      return declineRecovery(conversationId, reason, 'this answer already spent its error reload');
    }
  ...
  if (reason === 'silence') {
    ...
    if (proven && silenceEpisodeHolds(proven.turnKey, live) &&
        (proven.attempts >= SILENCE_RELOAD_ATTEMPTS || proven.episodeAttempts >= SILENCE_EPISODE_CEILING)) {
      if (!proven.told) {
        proven.told = true;
        ...
        void recordNote(
          sessionId,
          `Stopped reloading this chat: ${proven.episodeAttempts} reloads each came back with it still broken — ` +
...
async function noteRecoveryObservations(
  ...
  if (!completedFinal && !thinkingFailed && !mcpTerminal && !awaitingSilenceRefresh && (proTerminal || (activity.terminal && terminalGrant?.model !== 'pro'))) {
    ...
    } else if (lastEnd === 'failed' && sessionId) {
      // 2.1.0 dropped the `settledThinkingFailure` test here: the enclosing condition now
      // carries `!thinkingFailed`, so it said the same thing twice. The mark is ours — a turn
      // that ends failed with nothing after it is a chat nobody would otherwise report.
  ...
  for (const item of observations) {
    ...
    // across unrelated trouble.
    {
      ...
      // The per-failure budget below is deliberate and stays: a chat whose trouble is different
      // every time has not answered anything the same way, and muting its liveness check is how
      // a wedged chat went unnoticed before this watchdog existed.
      //
      // But the wording is not a reliable name for the trouble. Measured on 2026-09-12, one turn
      // reported a single wedge as "Connection interrupted" three times and then as "Message
      // delivery timed out" twice — and two variants of that second message, with and without a
      // trailing "Retry", already counted as separate failures. Each switch reset the per-failure
      // count, so the chat could never exhaust its budget at all.
      //
      // So the per-failure count keeps its meaning and a second, wording-blind count bounds the
      // episode. Varied trouble still buys more attempts than identical trouble; nothing buys
      // unlimited ones.
      if (proven && silenceEpisodeHolds(proven.turnKey, live)) {
        proven.attempts = proven.error === error ? proven.attempts + 1 : 1;
        proven.episodeAttempts += 1;
        proven.error = error;
      } else silenceProvenUseless.set(conversationId, { turnKey, error, attempts: 1, episodeAttempts: 1, told: false });
    }
 ...
 * canonical assistant message keyed by ChatGPT's own id, and — the reason this matters —
 * the `[[CLF-RESUME]]` marker that `markedContinuationTurns()` reads to commit a handoff. When
 * it stops delivering, a replacement chat works normally and its handoff can never commit,
 * and from here that is indistinguishable from a page that is merely quiet.
 *
 * Measured here between 2026-09-14 and 2026-09-17: `page_tool` observations fell from 5.3 per
 * turn to 0.08, one event across 1,253 tool calls on the last day, while `turn_start` and
 * `turn_end` arrived throughout — so the isolated world was alive the whole time. Five handoffs
 * ended `dispatched-unresolved` with the brief demonstrably in a chat that was on its 272nd
 * tool call. Nothing in the log said why, because nothing reported on this half at all.
 *
 * `absent` and `empty` are the two faults, and telling them apart is the whole point: absent is
 * an injection that is not landing, which this app can act on; empty is a helper that answers
 * and finds nothing, which is a change in what it reads and cannot be fixed from here. Said
 * once per chat per state change, so a page that stays broken does not repeat itself.
 */
...
async function inspectSilentChats(now: number): Promise<{ queued: boolean; spent: string[] }> {
  ...
  for (const [conversationId, grant] of [...activeUntil, ...restored]) {
    ...
    // A real grant arriving mid-await supersedes either kind of candidate. Absence is not a
    // mismatch: a restored or handed-back ticket is considered as a candidate before it is ever
    // granted, so an exact `!==` against an empty map discards it on the pass that created it.
 ...
 *
 * Expressed against the longest silence window rather than as a flat number, because a grace
 * shorter than the window it guards protects nothing: 2.1.0 made an unproven-model chat wait
 * the full Pro window, and a five-minute grace under a ten-minute window never decided anything.
 */
const RUNNING_CALL_GRACE_MS = PRO_SILENCE_MS + 2 * 60_000;
...
function commandDeadlineDelay(command: Command, now = Date.now()): number | null {
  ...
  if (waitingForRevivalReadiness(command)) return null;  if (command.spec.type === 'revive') return revivalDeadlineAt(command) - now;
