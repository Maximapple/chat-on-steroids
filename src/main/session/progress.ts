...
export function conversationProgress(conversationId: string) {
  let workers: { total: number; active: number; finished: number; failed: number; names: string[]; failedNames: string[] } | null = null;
  try {
    ...
    if (snapshot.self.role === 'prime') {
      ...
      if (list.length) workers = { total: list.length, active: active.length,
        ...
        // Same shape and the same bound as the running names beside it. The caption could say
        // that a worker needs attention and not which one, while the label was right here.
        failedNames: failed.slice(0, 3).map(agent => (agent.label || 'Worker').slice(0, 60)) };
    }
